...
public:
    ...
    void BFS(vector<int>& signalReceivedAt, int k) {
        ...
        while (!q.empty()) {
            ...
            for (pair<int, int> edge : adj[currNode]) {
                ...
                if(signalReceivedAt[currNode]+time < signalReceivedAt[neighborNode]){
                  q.push(neighborNode);
    ...
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        // Step 1. Build the adjacency list. Already done for you!
        ...
        // Hint : You can use INT_MAX from climits
        ...
        int answer = *max_element(signalReceivedAt.begin(), signalReceivedAt.end());
...
int main(){
    ...
    cout << "Time taken for all nodes to receive signal " << answer;
    return 0;
