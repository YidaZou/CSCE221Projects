...

         {
            ...
            for (int nextNode = 0; nextNode < n; ++nextNode) {
                ...
                if(!inMST.at(nextNode)){
                    int newWeight = abs(points.at(currNode).at(0) - points.at(nextNode).at(0)) + abs(points.at(currNode).at(1)-points.at(nextNode).at(1));
                    heap.push({nextNode, newWeight});
                }
            }
        }
...
int main() {
    vector<vector<int>> points = {{0, 0}, {2, 0}, {3, 10}, {5, 2}, {7, 0}};
